...
- 프로토콜과의 괴리: 프로토콜은 RTG 3 partner × 15 trials을 요구하지만 현재 코드는 이를 반영하지 않음
...
**요구사항:**
1. 하나의 YAML 파일(`game_config.yaml`)에 RTG 설정을 포함한다.
2. RTG의 구조적 파라미터(trials, blocks, endowment, multiplier 등)와
...
4. Pydantic으로 validation할 수 있도록, 각 필드에 타입과 제약조건을 주석으로 명시한다.
5. 예시 값은 프로토콜의 기본값(RTG multiplier=3, partner types 3종)을 사용한다.
...
4. Firestore 저장 스키마를 codebook의 RECORDED 변수와 일치시킨다.
   특히 다음 필드를 반드시 포함:
...
2. **Codebook 일치:** Firestore에 저장하는 필드명은 `Integrated_Codebook.md`의 변수명과 정확히 일치시킨다. 현재 코드의 비표준 필드명은 codebook 표준(`amount_sent`, `partner_return_amount` 등)으로 교체한다.
